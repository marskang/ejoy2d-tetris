...
local uiGameOverX = gameFrameW / 2 + gameFrameX
local  uiGameOverY = gameFrameH / 2 + gameFrameY
...
local btnStartCoord = {x = 768, y = 589, scale = 1}
local btnPauseCoord = {x = 870, y = 589, scale = 1}
...
function game.drawframe()
	...
	ui_next:draw()
	if isGameOver then
		ui_gameover:ps(uiGameOverX,uiGameOverY)
		ui_gameover:draw()
	end
	if isPause and isStarting == false 
		and isGameOver == false then
		ui_pause:ps(uiGameOverX,uiGameOverY)
		ui_pause:draw()
	end
	if isStarting and isGameOver == false then
		startAni:ps(uiGameOverX,uiGameOverY)
		startAni:draw()
	end
end
